...
class VAE(keras.Model):
    def __init__(self, encoder, decoder, _classifier = None, _auth = None, beta = 0.001, alpha = 0.1, gamma = 1, scale_included=True, loss_func = "MSE", use_stats=False,**kwargs):
        ...
        self.loss_trackers = {}
        for loss_name in ["total_loss", "reconstruction_loss", "kl_loss", "classification_loss", "feature_loss", "reencoding_loss", "auth_loss"]:
    ...
    def get_losses(self, data, reconstruction, z_mean, z_log_var, z, y_user_one_hot, training=True):
        ...
        if self.auth_on:
            ...
            auth_loss = kl - kl # i.e. zero
        
        loss = reconstruction_loss + self.beta * kl + self.gamma * feature_loss + self.alpha * auth_loss
        
        loss_dict = {"total_loss" : loss, "reconstruction_loss" : reconstruction_loss, "kl_loss": kl, "feature_loss": feature_loss, "auth_loss":auth_loss}
        return loss_dict
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss_dict = self.get_losses(data, reconstruction, z_mean, z_log_var, z, y_user_one_hot)
    ...
    def test_step(self, data):
        ...
        losses = self.get_losses(data, reconstruction, z_mean, z_log_var, z, y_user_one_hot, training=False)
        return losses
    ...
    def lb_keogh(self, ts1, ts2, w=10):
        ...
        above_sum = tf.reduce_mean((ts2[above]-u[above])**2)
        below_sum = tf.reduce_mean((ts2[below]-l[below])**2)
...
def get_auth_model(input_dim):
    ...
    x = Conv2D(100,(2,6),strides=(1,1), kernel_regularizer=l1(0.01))(x)
