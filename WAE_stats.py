...
class Sampling(layers.Layer):
    ...
    def call(self, inputs):
        ...
        return z_mean + epsilon # tf.exp(0.5 * z_log_var) * epsilon
    
    
class WAE_stats(keras.Model):
    def __init__(self, latent_dim, alpha = 0.1, beta = 0.001, gamma=0.1, delta=0.1, **kwargs):
        super(WAE_stats, self).__init__(**kwargs)
        ...
        self.kl_loss_tracker = keras.metrics.Mean(name="wae_loss")
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            z_mean_features = (self.vae_featurize(z_mean_reconstruction) - self.feature_means) / self.feature_stds
            true_features = (self.vae_featurize(data) - self.feature_means ) / self.feature_stds
            
            feature_loss = tf.reduce_mean(keras.losses.mean_absolute_error(z_mean_features, true_features))
            ...
            A = tf.concat([z[:,5:],zis[:, 5:]], 0)
            r = tf.reduce_sum(A*A, 1)

            r = tf.reshape(r, [-1, 1])
            D = r - 2*tf.matmul(A, tf.transpose(A)) + tf.transpose(r)
            D = tf.math.sqrt(tf.clip_by_value(D, clip_value_min=1e-4, clip_value_max=1e8))
            ...
            total_loss = reconstruction_loss + self.beta * wae_loss + self.alpha*auth_loss + self.gamma*feature_loss
...
def get_decoder(latent_dim):
    ...
    x = layers.TimeDistributed(layers.Dense(6))(x)

    x = layers.Conv1D(6,4, padding="same")(x)
